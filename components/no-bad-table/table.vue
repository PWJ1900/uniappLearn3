<template>
	<view class="no-bad-table-wrap" :class="[tableHeight!='auto'?'fix-height':'',celCenter?'td-center':'']">
		<view class="table_box_big" :style='tableHeightUseGet'>
											 ...
											 width: countColspanWidth(item,tdItem,index,tdItemIndex,true)}'>
													...
													<template v-if="tdItem.$operateList">
														<template v-for="btn in tdItem.$operateList">
															<!-- #007aff -->
															<button :class="[btn.styles?btn.styles:'']" v-bind:style="item['pz']=='补拍'?{ 'background-color':'#ff0000',padding: '2px 5px',fontSize:'12px',lineHeight:'1.2',display:'inline-block'}:{padding: '2px 5px',fontSize:'12px',lineHeight:'1.2',display:'inline-block'}"
															 @click="pullEvent(btn.event,{row:item,index:index})" type='primary' size="min"   :key="btn.id">{{item['pz']=='补拍'?'补拍':'已拍'}}</button>
														</template>
													</template>
					...
					<view class="fixed-left" v-if="columnsFixedLeft[0]||(selection=='mulit'&&fixedCheckbox)">
						...
						>
							...
							<view class="td selection fixed-td" v-if="selection=='mulit'&&fixedCheckbox" :style='{width:selectionTdWidth,height:tdHeight+"px"}'>
								<view :class="['td_wrap']" :style='{width:selectionTdWidth,height:tdHeight+"px"}'>
									<checkbox :value="`${index}`" color="#999"  :disabled="checkBoxList[index].$disabled" :checked="checkBoxList[index].$checked"
									 style="transform:scale(0.7)" />
								</view>
							</view>
							<!-- 普通固定左边 【-->
			...
			* */
			checkboxChange(e) {
				...
				if (val.length == this.allCheckBoxAbledLen) {
					...
					this.checkBoxList = this.checkBoxList.map((item,index) => {
						if (val.indexOf(index.toString())>-1) {
							item.$checked = true
						} else {
							item.$checked = false;
						}
